...
class ViewMailActivity : BaseActivity() {
                ...
                override fun success(resp: String?, status: Boolean?) {
                    if (status!!) {
                        if (resp!!.contains(".pdf")) {
                            startActivity(Intent(this@ViewMailActivity,
    ...
    private fun bodyMsgFwd(body: String, date: String): String? {
        val s = "<br>" +
                ...
                "On ${UtilsDefault.replyDayMail(date)}, ${UtilsDefault.replyTime(date)} at ${
                    UtilsDefault.todayDate(UtilsDefault.localTimeConvert(date))
                }<br>Subject: $sub<br>To: &lt;<a href=\"$to\">$to</a>&gt;<br></div><br><br>" +
                "$body<br></div></div>\r\n"
        return s
    }
    ...
    fun bodyMsg(values: String, date: String): String {
        val s = "<br>" +
                "<div class=\"gmail_quote\">" +
                "<div dir=\"ltr\" class=\"gmail_attr\">On ${UtilsDefault.replyDayMail(date)}, ${
                    UtilsDefault.replyTime(date)
                } at ${UtilsDefault.todayDate(UtilsDefault.localTimeConvert(date))} &lt;<a href=\"mailto:$from\">$from</a>&gt; wrote:<br></div>" +
